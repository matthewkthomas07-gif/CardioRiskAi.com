...
4. Chat with the assistant — type **start** and answer in plain language (type **skip** for hospital-only tests you do not have).
...
The chat bot uses **plain language** (no medical codes). See **`medical_glossary.json`** and the website glossary for how to count each value at home.

| Topic | At home? | Tip |
|-------|----------|-----|
| Age, sex, chest pain | Yes | Answer in words (e.g. `none`, `typical`) |
| Blood pressure | Yes | Home cuff, top number |
| Fasting sugar, exercise chest pain | Often | Yes/no or home glucose meter |
| Max heart rate | Often | Pulse after stairs, or 220 − age |
| Cholesterol, ECG, stress test, imaging | Usually clinic | Type **skip** if unknown — less precise estimate |

Validated accuracy target: **95%** on the primary clinical benchmark (see `model_metrics.json`).
